#!/usr/bin/env python3
# Expected output: This script reads numbers from a file called 'numbers.txt'
# calculates their sum and average, writes the results to 'results.txt'
# and prints a confirmation message. It should handle errors gracefully.
        ...
        with open(filename, 'r') as f:
            for line in f:
                ...
                if stripped == "":
                    continue
                try:
                    num = float(stripped)  # logic error:cast int decimal exist
                    numbers.append(num)
                except ValueError:
...
def calculate(numbers):
    ...
    for n in numbers:
        total += n
    avg = total / len(numbers)  # Logic error: div by 0 instead of len(numbers)
    return total, avg
